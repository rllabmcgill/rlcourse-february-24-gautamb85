...
def plan(Q,lr,y,model_nextS,model_nextR,plan_steps=15):
    ...
    while psteps<=plan_steps-1:
        ...
        Q[rstate,ract] = Q[rstate,ract] + 0.001*(r_hat + 0.001*np.max(Q[s_hat,:]) - Q[rstate,ract])
        psteps+=1
...
def train(env,lr=0.1,y=0.99,num_episodes=10000,planning=False):
    ...
    for i in range(num_episodes):
        ...
        while j < 2500:
            ...
            if done:
                if len(score) < 100:
                    score.append((r+time_reward))
                else:
                    score[i%100] = r+time_reward
...
def evaluate(env,Q):
    ...
    for i in range(1000):
        ...
        for t in range(2500):
            ...
            if done:
                if len(score) < 100:
                    score.append((reward+time_r))
                else:
                    score[i%100] = reward+time_r
